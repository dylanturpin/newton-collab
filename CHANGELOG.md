...
- Add opt-in ``contact_patch_wrench`` parameter to `SolverFeatherPGS`: each coplanar matrix-free contact patch (at least two contacts, agreeing normals, coplanar witness points; up to eight contacts per pair) is solved as one block — total normal force at the patch's load-bearing centroid, a tangential friction pair whose cone uses the whole patch's Coulomb budget, two tipping-moment rows clamped so the center of pressure stays within an eight-sided bound of the load-bearing support (support weight fades smoothly with separation), and, when friction is active, a torsional friction row (three rows without friction). Ineligible contacts keep classic per-contact rows. Making the patch moment an explicit solver variable removes the moment-redistribution lag that destabilizes tall box stacks at low iteration counts. Requires ``friction_mode="current"`` and pair-sorted contacts. Disabled by default with an unchanged launch sequence.
