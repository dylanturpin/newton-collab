...
- Add opt-in ``contact_patch_wrench`` parameter to `SolverFeatherPGS`: each coplanar matrix-free contact patch is solved as one six-row block — total normal force at the patch's load-bearing centroid, a tangential friction pair whose cone uses the whole patch's Coulomb budget, two tipping-moment rows clamped so the center of pressure stays within an eight-sided bound of the load-bearing support (separated contacts grant no support), and a torsional friction row. Making the patch moment an explicit solver variable removes the moment-redistribution lag that destabilizes tall box stacks at low iteration counts. Requires ``friction_mode="current"`` and pair-sorted contacts. Disabled by default with an unchanged launch sequence.
