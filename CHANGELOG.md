...
- Add opt-in `reduce_contacts_body_pairs` parameter to `CollisionPipeline` that compacts rigid contacts per body pair and contact-normal bin after the narrow phase, keeping each group's deepest contact plus the spatial extremes of its near-touching footprint. Bounds the registered contact count of multi-shape bodies by the contact-patch structure instead of the collider decomposition; `reduce_contacts_body_pairs_depth_window` tunes the participating gap band. Disabled by default.
