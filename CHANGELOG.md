...
- Add `bilateral_preelimination_include_mimics` parameter to `SolverFeatherPGS` (default `True`): when disabled, bilateral pre-elimination folds only the connect (loop-closure) rows into the Schur block and leaves mimic rows to the iterative sweep, for A/B isolation of near-dependent mimic couplings. Add opt-in `debug_record_residuals` (default `False`) recording per-visit dense-row residuals into `pgs_residual_log` for solver diagnostics.
...
- Import passive joint springs from USD in `ModelBuilder.add_usd()`: `mjc:stiffness` / `mjc:springref` (MjcJointAPI, as authored by the MuJoCo USD converter) and the new Newton custom attributes `newton:springStiffness` / `newton:springRef` now populate `Model.joint_spring_stiffness` / `joint_spring_ref` on revolute, prismatic, and D6 joints. `mjc:springref` angle units follow the scene-level `mjc:compiler:angle` token (schema default: degrees); Newton-authored angular values use the USD per-degree convention. `ModelBuilder.add_joint_revolute()` and `add_joint_prismatic()` gain matching `spring_stiffness` / `spring_ref` keyword arguments.
