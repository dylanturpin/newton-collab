...
- Add `pgs_contact_regularization` to `SolverFeatherPGS`: a dimensionless regularizer on contact rows of every route (matrix-free, dense, propagation) that makes statically indeterminate normal-force splits unique and damps the sweep at a documented resting sag of `g * a * dt^2 / pgs_beta` per loaded row. Positive-gap speculative rows and firing restitution rows are solved rigid; the velocity-only pass is exempt. Defaults to 0, the exact rigid law.
