...
- Add rigid-contact restitution to the `SolverFeatherPGS` matrix-free velocity pass. Reached contacts use the incident pre-contact relative normal velocity and the arithmetic-mean shape coefficient to enforce Newton's impact law, while unreached speculative contacts keep their closing allowance. The position solve remains unchanged, all immediate/propagation/colored/fused contact routes share the same law, and `restitution_velocity_threshold` (default 0.5 m/s) suppresses low-speed resting bounce. Positive restitution requires `pgs_velocity_iterations > 0`; in-place coefficient updates remain compatible with an already-captured graph when the velocity pass was enabled at construction.
