...
- Fix `SolverFeatherPGS` halting a falling body in mid-air when `pgs_velocity_iterations` is nonzero. The final velocity pass dropped the speculative allowance from contacts that were inside the collision margin but carrying no impulse, forbidding any approach instead of permitting closure by the remaining gap. Contacts that are genuinely in contact are unaffected, as is the default zero-iteration path.
