...
- Add opt-in `reduce_contacts_body_pairs` parameter to `CollisionPipeline` that compacts rigid contacts after the narrow phase, keeping each group's deepest contact plus the spatial extremes of its footprint, where a group is a (body pair, material class, contact-normal bin, spatial cell). Bounds the registered contact count of multi-shape bodies by the contact-patch structure instead of the collider decomposition. Companion parameters: `reduce_contacts_body_pairs_cell` subdivides each pair spatially so separated same-normal patches stay fully represented, `reduce_contacts_body_pairs_hysteresis` keeps slot winners stable across steps on curved support (nonzero by default; reset via `CollisionPipeline.reset_body_pair_reduction_history()` at episode boundaries; rebuild material classes with `CollisionPipeline.refresh_body_pair_reduction_groups()` after runtime material mutation), `reduce_contacts_body_pairs_hashtable_factor` sizes the group table, and `reduce_contacts_body_pairs_verify` enables a per-frame certificate. Resource exhaustion (narrow-phase overflow or group-table saturation) deterministically falls back to the full unreduced set for that frame. Telemetry via `CollisionPipeline.body_pair_reduction_stats()` (cleared with `clear_body_pair_reduction_stats()`). Capturing `collide()` into a CUDA graph binds the pipeline to that exact contacts buffer: warm the buffer up with one ordinary `collide()` first, use no other buffer while the binding is live, and call `release_body_pair_reduction_capture()` after destroying the graph. Known fidelity tradeoffs: the footprint hull is sampled by six fixed scan directions (support/tipping preserved only to scene-dependent sampling tolerances, and coincident duplicate contacts are all kept), and torsional patch friction is biased upward (measured 1.34x the uniform-pressure value on a circular patch; shape-dependent). Rejects hydroelastic configurations at construction, and solvers must declare `supports_reduced_contacts` (conformance-tested for `SolverFeatherPGS`; `SolverXPBD` is conformance-tested with a documented residual -- reduced curved-collider support can sustain a roll oscillation at low iteration counts, decaying as iterations rise; all other solvers reject reduced buffers at `step`). Disabled by default.
