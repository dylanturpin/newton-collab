...
- Add rigid-contact restitution to `SolverFeatherPGS` in matrix-free mode. Reached contacts use the incident pre-contact relative normal velocity and the arithmetic-mean shape coefficient to enforce Newton's impact law, while unreached speculative contacts keep their closing allowance. The position solve remains unchanged, all immediate/propagation/colored/fused contact routes share the same law, and `restitution_velocity_threshold` (default 0.5 m/s) suppresses low-speed resting bounce. Positive construction-time restitution automatically enables one velocity-only correction iteration; `pgs_velocity_iterations` can request a larger total count. In-place coefficient updates remain graph-compatible when restitution storage was provisioned at construction.
