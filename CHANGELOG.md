...
- Fix `SolverFeatherPGS` halting a falling body in mid-air when `articulated_contact_response` is `"propagation"` or `"propagation-colored"` and `pgs_velocity_iterations` is nonzero. The final velocity pass dropped the speculative allowance from contacts that were inside the collision margin but carrying no impulse, forbidding any approach instead of permitting closure by the remaining gap. The same rule is now applied consistently across the dense, matrix-free and propagation contact routes. The default zero-iteration path is unchanged.
