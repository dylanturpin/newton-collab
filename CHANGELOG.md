...
- Add trajectory inverse kinematics: `newton.ik.IKSolverTrajectory` optimizes all frames of one or more joint trajectories jointly, reusing existing per-frame IK objectives (with one target per frame) and solving the block-banded Gauss-Newton system with either a batched block-tridiagonal Cholesky (`linear_solver="direct"`) or a block-Jacobi-preconditioned conjugate-gradient (`linear_solver="cg"`) backend. New temporal objectives couple consecutive frames: `IKObjectiveSmoothness` (finite-difference velocity/acceleration/jerk), `IKObjectiveVelocityLimit`, and `IKObjectiveJointReference`; `fixed_frames` anchors selected frames. See the `ik_trajectory` example.
...
- Add `frame_counts` to `newton.ik.IKSolverTrajectory`: batch trajectories of unequal length in one solve. Each trajectory's temporal stencils, costs, and linear system stop at its own last frame, rows past a trajectory's frame count are inert, and on the `direct`/`spike` backends every member solves bitwise identically to its unpadded single-trajectory solve. The resolved per-trajectory `spike` partition plan is readable via the new `IKSolverTrajectory.spike_partition_counts`.
...
- Add `newton.ik.IKObjectiveFootContact`, a contact-gated trajectory-IK objective pinning contact points to per-frame anchor targets during labeled stance (PyRoKi's floor-contact cost)
...
- Change the `newton.ik.IKSolverTrajectory` `direct`/`spike` factorization to lean streaming kernel passes: on problems the previous fused kernels could run, results are numerically equivalent but no longer bitwise-identical to prior releases (the subtract-accumulate GEMM rounds differently); pass `refine_iterations=1` where a backend-independent solution is needed. Constructing a `direct`/`spike` solver now also compiles its factorization kernels eagerly to check their shared-memory fit, moving that compile latency from the first `step()` into the constructor.
...
- Change `newton.ik.IKObjectiveTemporal.set_trajectory_layout` to take a `frame_counts` keyword argument, which `newton.ik.IKSolverTrajectory` now always passes; temporal-objective subclasses overriding the previous two-argument signature must accept the new keyword (add `frame_counts=None` or `**kwargs`).
...
- Fix `newton.ik.IKSolverTrajectory` CG-backend under-convergence for multi-trajectory solves (`n_problems > 1`) with long horizons: per-trajectory CG dot products on CUDA now use a deterministic reduction whose accuracy no longer degrades with horizon length, so batched solves converge like the equivalent single-trajectory solves at the same `cg_iterations` budget. Batched solves at or below 65,536 scalar dofs per trajectory are bitwise-unchanged.
- Fix `IKSolverTrajectory` `direct`/`spike` backends failing to launch on devices whose CUDA shared memory cannot hold the fused factorization kernels at production superblock sizes (e.g. 35 DoF with an acceleration-smoothness stencil): the factorization now streams through the existing global workspaces in lean kernel passes that keep at most five superblock tiles in shared memory, and problems that still cannot fit raise `newton.ik.IKSharedMemoryError` at construction instead of a CUDA launch error inside `step()`.
