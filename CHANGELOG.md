...
- Add rigid-contact restitution to `SolverFeatherPGS` in dense, split, and matrix-free modes. The ordinary normal row freezes the incident pre-contact relative normal velocity and, for a sufficiently fast contact predicted to reach the surface during the step, replaces geometric/speculative bias with Newton's rebound target using the arithmetic-mean shape coefficient. Other contacts retain their existing speculative or Baumgarte law. `restitution_velocity_threshold` defaults to 0.5 m/s, `pgs_velocity_iterations` remains exactly user-controlled and optional, every immediate/propagation/colored/fused route shares the same impact law, and in-place coefficient updates remain CUDA-graph compatible. As in conventional PGS, rebound velocity applies over the whole step, giving a first-order impact-phase position offset without a time-of-impact substep. Matrix-free warm starts now install cached impulse response into the live velocity before iteration, preventing stale normal impulses from attracting separating contacts.
...
- Extend the existing `CollisionPipeline.reduce_contacts` entry point with `CollisionPipeline.ContactReductionConfig`. Boolean values retain their released meaning exactly: they only control producer-side mesh/heightfield reduction. The config adds an opt-in `body_pairs` postpass for compound-body patches, with cell size, hysteresis, group-table headroom, and verification settings grouped in the same object instead of five additional constructor keywords. The group table is sized from the model's own contact-pair topology scaled by `body_pair_hashtable_headroom`, so its capacity tracks how many body pairs a scene can have in contact rather than how large its contact buffer is. The body-pair stage keeps one depth representative and up to six sampled footprint representatives among the contacts delivered by the narrow phase; nonzero hysteresis may retain an incumbent within its configured score margin, and coincident contacts with identical packed winner keys may exceed the nominal slot count. It remains disabled by default, requires producer reduction when mesh/heightfield paths are active, rejects active hydroelastic contacts, contact matching, and FeatherPGS warm start, and is conformance-supported by the default `SolverFeatherPGS` configuration through `supports_body_pair_reduced_contacts`. Overflow and bounded-probe failures keep the materialized buffer unchanged, telemetry remains available through `body_pair_reduction_stats()`, and CUDA graph capture retains its exclusive warmed-buffer lease. Known fidelity tradeoffs remain scene-dependent sampled-support error and possible upward torsional-friction bias.
- Add opt-in `propagation_same_articulation_rows` parameter to `SolverFeatherPGS` that routes same-articulation link-link contacts to propagation rows with exact cross operational-space response (previously always dense). Defaults to off; requires `articulated_contact_response="propagation"`.
- Add mimic constraint support to `SolverFeatherPGS` (matrix-free mode): `Model.constraint_mimic_*` couplings between REVOLUTE/PRISMATIC joints of the same articulation are enforced as bilateral constraint rows, and `coef0`/`coef1`/`enabled` are read live so runtime updates take effect without solver re-initialization. Cross-articulation and multi-DoF mimics warn and are ignored.
- Add connect (loop-closure) constraint support to `SolverFeatherPGS` (matrix-free mode): trailing BALL loop joints — how the MJCF importer realizes `connect` equalities — are excluded from the kinematic tree and enforced as three normalized bilateral rows pinning the joint's anchors together, closing kinematic loops such as gripper four-bar linkages. FIXED (weld) loop joints and non-`immediate` articulated contact response warn or raise; propagation-mode support is deferred.
- Add opt-in `enable_bilateral_preelimination` parameter to `SolverFeatherPGS` (matrix-free mode, default off): the mimic and connect rows of each articulation are pre-eliminated through a small per-articulation Schur complement, so every other constraint row — contacts especially — sees the closed-loop effective mass exactly (`Y' = Y - Y_B S^-1 (J_B Y)`), and the predictor velocity is projected onto the bilateral manifold once per step in place of iterative enforcement. Loop closures become iteration-independent (measured: four-bar anchor gap 0.5 mm -> 0.0002 mm at 2 PGS iterations). The Schur diagonal uses relative regularization so planar closures' near-dependent axes stay SPD at any mass scale. Requires `pgs_velocity_iterations == 0` and at most 8 bilateral rows per articulation; unsupported configurations warn and fall back to iterative rows.
...
- Add `enable_mimic_constraints` and `enable_connect_constraints` parameters to `SolverFeatherPGS` (both default `True`) that toggle the mimic and connect (loop-closure) equality row families independently, for A/B comparison against the pre-equality solver behavior. Disabling warns; loop joints remain excluded from the kinematic tree either way, so disabling `enable_connect_constraints` leaves closed linkages open but stable.
- Add generic passive joint springs: `Model.joint_spring_stiffness` / `Model.joint_spring_ref` (per-DOF, default 0) with matching `ModelBuilder.JointDofConfig.spring_stiffness` / `spring_ref` fields; the MJCF importer now maps joint `stiffness` / `springref` into them (the solver-private `mujoco:` copies are unchanged, so `SolverMuJoCo` behavior is identical).
- Add passive joint spring support to `SolverFeatherPGS`: springs are applied as an explicit torque `k * (ref - q)` on REVOLUTE/PRISMATIC/D6 DOFs during the inverse-dynamics pass, gated by `enable_joint_springs` (default True; springs on other joint types warn and are ignored).
...
- Add `SolverFeatherPGS.joint_qd_public_convention`, declaring that FeatherPGS stores free-joint `joint_qd` in Newton's public `(v_com_world, omega_world)` convention. Integration layers that refresh maximal body state from joint state should dispatch on it to select `newton.eval_fk`; applying the Featherstone-internal conversion helper instead adds `omega x x_com_world` of phantom linear velocity to `State.body_qd` that grows with the body's distance from the world origin.
...
- Infinite planes now use half-space AABBs in the collision broad phase (clamped at the surface along axis-aligned normal components) instead of a bounding-sphere fallback that overlapped every shape. Shapes far from an axis-aligned plane are no longer broad-phase candidates against it; contact results are unchanged. The clamp engages only when the plane's residual lateral tilt times the supported reach (10 km about the origin) stays within half the shape's contact gap, carrying that rise as slack — a nearly-aligned plane (for example a floor deliberately tilted a twentieth of a degree) keeps the unbounded extent instead of silently dropping resting contacts far from the plane anchor.
- The narrow-phase GJK distance query now stops iterating as soon as a support plane proves two shapes are separated beyond the contact threshold (margin + gap), instead of converging to the exact distance for pairs whose contact would be rejected anyway. Contact results are unchanged.
- Remove the `SolverFeatherPGS` `hinv_jt_chunk_size` parameter. The tiled `H^-1 J^T` chunk width is now derived per DOF size from the device's shared-memory capacity at construction (largest fitting candidate of 64/32/16/8/4/2/1), superseding the fixed-width knob while keeping its guarantee that `dense_max_constraints` may exceed the per-block shared-memory limit. `hinv_jt_kernel="tiled"` raises `ValueError` only when no chunk width fits the device.
...
- Fix `SolverFeatherPGS` halting a falling body in mid-air whenever `pgs_velocity_iterations` is nonzero. The final velocity pass now evaluates each positive-gap row against the linearized position solution: contacts whose end gap exceeds a 1e-6 m numerical slop keep their speculative closing allowance, while contacts within the slop use the unbiased velocity law. The same classification is applied across the dense, matrix-free, and propagation paths, does not depend on stored impulse magnitude or history, supports warm starting, and leaves the zero-restitution zero-iteration path unchanged.
- Fix `SolverFeatherPGS` corrupting device memory and NaN-ing when a model contains loop joints or any articulation placed after one: the composite-inertia/CRBA chain indexed body-sized spatial workspaces by joint index, which only coincides with the child-body index in loop-free models. The chain now indexes through `joint_child`, and loop joints are excluded from the kinematic tree (FK, inverse dynamics, mass matrix, body maps) via a per-articulation tree-prefix bound.
- Fix `SolverFeatherPGS` ignoring passive joint damping (`Model.joint_damping`): it is now applied as an explicit torque `-c * qd` in the inverse-dynamics pass, matching `SolverFeatherstone` semantics (drive gains stay on the implicit augmented path). Pass `enable_joint_passive_damping=False` to recover the previous behavior.
...
- Fix `SolverFeatherPGS` building contact, friction, and velocity-limit rows against a free-root velocity its integrator never realizes. The integrator adds the free base's transport term (`omega x v`) to the linear coordinate, but the velocity predictor and the post-solve velocity-to-acceleration conversion stayed component-wise, so for a base that spins while it translates every constraint row saw a centre-of-mass velocity off by `dt * (omega x v)`, and the realized velocity differed from the solved one by the same amount (contact-free runs cancel exactly, which momentum tests cannot see). Both conversions now carry the term.
- Fix the `SolverFeatherPGS` free-joint integrator ignoring a non-identity `child_xform` in its anchor-to-COM lever: a force-free spinner with an offset joint anchor orbited its own centre of mass instead of holding it still. The root anchor is now reconstructed from the directly integrated centre of mass (as `SolverFeatherstone` does), so a force-free COM holds to roundoff instead of leaking first-order truncation. Descendant free joints (parent is a body, not the world) now integrate component-wise again, restricting the root transport rule to actual roots until the parent-frame transport is derived for them.
- Fix `SolverFeatherPGS` creating linear momentum in rotating floating-base articulations with more than one link. The free base is solved in a world-aligned frame centred on a material point of the root body, and three things disagreed about that point: the root link's linear inertial wrench was zeroed, the free-base linear coordinate was integrated without its transport term, and the public-to-internal shift referenced the root body's origin rather than its centre of mass. Each partly compensated the others, so a rotating articulation gained momentum and energy with no contacts, joint limits or constraint rows involved, and the error did not shrink with the timestep. Momentum is now conserved to the integrator's own order, matching `SolverFeatherstone` on the same models. Single free bodies are also corrected when they tumble with a centre of mass offset from the body origin (the public-to-internal shift leaked momentum whenever the angular velocity changed within a step); only single bodies whose centre of mass sits at the body origin, or whose spin is about a principal axis, reduce to the previous behaviour exactly.
