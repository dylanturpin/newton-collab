...
- Add opt-in `pgs_inner_substeps` parameter to `SolverFeatherPGS`: frozen-basis inner substeps (a TGS-style position loop) that build the contact problem once per `step()` and run N cycles of Gauss-Seidel iterations, joint integration at `dt/N`, and first-order contact-error re-measurement (`phi += dt*J*v` on the frozen Jacobians). Raises the effective contact-correction rate like true substeps while rebuilding the dynamics only once per step. The saving depends on where a scene spends its time: it is largest when the per-step build (mass matrix, Cholesky, Jacobians) dominates and small when the Gauss-Seidel sweep and contact work dominate, since those still run once per inner substep. Requires `pgs_mode="matrix_free"` with `articulated_contact_response="immediate"`, and rejects velocity iterations, warm starting and `pgs_debug`; defaults to 1 (off), leaving the launch sequence unchanged.
