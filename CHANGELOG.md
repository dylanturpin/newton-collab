...
- Fix `SolverFeatherPGS` halting a falling body in mid-air whenever `pgs_velocity_iterations` is nonzero. The final velocity pass dropped the speculative allowance from contacts that were inside the collision margin but carrying no impulse, forbidding any approach instead of permitting closure by the remaining gap. This affected the matrix-free and propagation contact routes alike; the same rule is now applied across the dense, matrix-free and propagation paths. The default zero-iteration path is unchanged. `pgs_warmstart`, `mf_warmstart` and the `IL_NEWTON_FPGS_MF_WARMSTART` override are rejected together with `pgs_velocity_iterations > 0`, since warm-start residue would make that classification unreliable.
