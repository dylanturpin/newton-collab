...
- Add `newton.utils.filter_static_self_collision_pairs()` and `newton.utils.find_static_self_collision_filters()`: sample an articulation's joint space to identify shape pairs that are broad-phase candidates in essentially every reachable configuration yet never come within contact range (e.g. adjacent finger links), and optionally exclude them via collision filter pairs. Opt-in preprocessing; pairs that can genuinely touch are never filtered.
...
- Infinite planes now use half-space AABBs in the collision broad phase (clamped at the surface along axis-aligned normal components) instead of a bounding-sphere fallback that overlapped every shape. Shapes far from an axis-aligned plane are no longer broad-phase candidates against it; contact results are unchanged.
