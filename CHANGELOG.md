...
- Add `SolverFeatherPGS` to the public `newton.solvers` export surface.
- Allow `SolverFeatherPGS.update_contacts()` to accept the optional state argument used by Newton manager integrations.
...
- Default the nightly dashboard to H1 Tabletop on first load, narrow the kernel-detail view to the final Newton ablation plus MuJoCo baseline, hide low-signal kernel rows below a 1% cutoff, and switch profiling artifacts into a stacked layout on tablet-width viewports.
