...
- Fix `SolverFeatherPGS` creating linear momentum in rotating floating-base articulations with more than one link. The correction that removes the body-frame Coriolis term was applied to the root link only, leaving every other link in a different reference-frame convention, so an articulation whose composite centre of mass sits away from its root body's centre of mass gained momentum and energy whenever its base rotated -- with no contacts, joint limits or constraint rows involved. The residual error is now first-order in the timestep instead of insensitive to it. Single-link articulations are unaffected: their frame origin already coincides with the composite centre of mass, so the correction reduces to the previous behavior exactly.
