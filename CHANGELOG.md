...
- Fix `SolverFeatherPGS` halting a falling body in mid-air whenever `pgs_velocity_iterations` is nonzero. The final velocity pass now evaluates each positive-gap row against the linearized position solution: contacts whose end gap remains positive keep their speculative closing allowance, while contacts whose end gap reaches zero use the unbiased velocity law. The same classification is applied across the dense, matrix-free, and propagation paths, does not depend on stored impulse magnitude or history, supports warm starting, and leaves the default zero-iteration path unchanged.
