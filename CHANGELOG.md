...
- Add opt-in `reduce_contacts_body_pairs` parameter to `CollisionPipeline` that compacts rigid contacts per body pair and contact-normal bin after the narrow phase, keeping each group's deepest contact plus the spatial extremes of its near-touching footprint. Bounds the registered contact count of multi-shape bodies by the contact-patch structure instead of the collider decomposition; `reduce_contacts_body_pairs_depth_window` tunes the participating gap band and `reduce_contacts_body_pairs_cell` subdivides each pair spatially so separated same-normal patches stay fully represented. The kept set is deterministic by construction (content-fingerprint winner selection, no sorting required); rejects hydroelastic configurations at construction, and solvers must declare `supports_reduced_contacts` (conformance-tested for `SolverFeatherPGS` and `SolverXPBD`; all other solvers reject reduced buffers at `step`). Disabled by default.
