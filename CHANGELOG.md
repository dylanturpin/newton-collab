...
- Add opt-in ``box_box_sat`` parameter to `CollisionPipeline` routing box-box pairs through the existing SAT reference-face `collide_box_box` primitive instead of GJK/MPR, with a 4-slot manifold whose contact identity comes from generation-time clipping features. Fixes GJK/MPR witness-point teleports (measured up to a face width per frame under micro-wobble), which otherwise surface as phantom deep-penetration contacts and violent impulse kicks when stacked boxes are disturbed. Disabled by default with an unchanged default pipeline.
...
- Fix `SolverFeatherPGS` silently dropping connect (loop-closure) rows when another articulation's joints interleave between an articulation's tree joints and its loop-closing joints — the ordering `add_usd` produces whenever the imported subtree contains a closed-loop robot plus any floating rigid body (the IsaacLab scene path). Loop joints are now classified via `Model.joint_articulation` and attributed to the articulation owning their child body instead of by joint-index range; previously the closed linkage fell open (measured on the Robotiq 2F-85: zero connect rows, 107 mm anchor gap, fingers collapsed) while flat single-asset imports worked by ordering luck.
