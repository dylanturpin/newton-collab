...
class SolverFeatherPGS(SolverBase):
    ...
    def __init__(
        ...
        # Thread-per-node colored path: one lane per coloring node, single warp
        # per world (block_dim = 32). FEATHER_PGS_MFGS_WARPS is no longer used
        # for the colored path (the prior warp-per-node W>1 variant was replaced).
        self._mfgs_warps = 1
    ...
    def _launch_matrix_free_gs_solve(
        ...
        *,
        ...
        # Solve-only micro-bench hook (FEATHER_PGS_MFGS_SOLVE_BENCH). Stash a
        # zero-arg closure that replays the EXACT MF-GS schedule launched this
        # step (same kernel, same live buffers: dense_rhs / mf_meta / colored CSR
        # are captured by reference and remain valid for the settled state). The
        # caller (probe) invokes mfgs_solve_bench() AFTER the step loop, OUTSIDE
        # any CUDA-graph capture, to time the GS kernel in isolation from the
        # temporary host-coloring cost. Behavior-neutral: only writes self attrs.
        if os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH", "0").lower() in {"1", "true", "yes", "on"}:
            self._mfgs_last_solve_replay = _run_schedule
            self._mfgs_last_block_dim = mf_block_dim
            self._mfgs_last_iterations = int(iterations)
            self._mfgs_last_schedule = str(self.pgs_schedule)

    def mfgs_solve_bench(self) -> dict | None:
        """Replay the last MF-GS solve schedule N times and report median ms.

        Times ONLY the matrix-free Gauss-Seidel solve kernel (the launch the
        solver issues every step), synchronized per replay, on the live settled
        buffers from the most recent ``step``. Must be called OUTSIDE any CUDA
        graph capture (a real-launch + device-sync loop is illegal under
        capture). Prints a single ``[SOLVE-BENCH]`` line and returns the medians.
        Returns ``None`` if no MF-GS launch has run yet (e.g. serial dense path).
        """
        import statistics as _stats
        import time as _time

        replay = getattr(self, "_mfgs_last_solve_replay", None)
        if replay is None:
            print("[SOLVE-BENCH] ERROR no MF-GS launch was recorded (set FEATHER_PGS_MFGS_SOLVE_BENCH=1)", flush=True)
            return None
        dev = self.model.device
        n_launches = max(int(os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH_N", "250")), 1)
        n_warmup = max(int(os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH_WARMUP", "50")), 0)
        variant = f"colored_W{self._mfgs_warps}" if self._mfgs_colored else "serial"
        try:
            ...
            med = _stats.median(samples)
            mn = min(samples)
            p10 = _stats.quantiles(samples, n=10)[0] if len(samples) >= 10 else mn
            print(
                f"[SOLVE-BENCH] variant={variant} colored={int(self._mfgs_colored)} "
                f"W={self._mfgs_warps} block_dim={self._mfgs_last_block_dim} worlds={self.world_count} "
                f"iters={self._mfgs_last_iterations} schedule={self._mfgs_last_schedule} "
                f"launches={n_launches} warmup={n_warmup} "
                f"median_ms={med:.5f} min_ms={mn:.5f} p10_ms={p10:.5f}",
                flush=True,
            )
            return {"variant": variant, "median_ms": med, "min_ms": mn, "p10_ms": p10}
        except Exception as _e:
            print(f"[SOLVE-BENCH] variant={variant} ERROR {type(_e).__name__}: {_e}", flush=True)
            return None
...
def _get_pgs_solve_mf_gs_kernel(
    ...
    # THREAD-PER-NODE colored path: one LANE owns one coloring node; a single
    # warp's 32 lanes process up to 32 independent nodes of a color in parallel.
    # block_dim is fixed at 32 (single warp / W=1); FEATHER_PGS_MFGS_WARPS is no
    # longer a tuning knob for the colored path (the prior warp-per-node W>1
    # variant was replaced). Serial path is always single-warp.
    MFGS_WARPS = 1
    if MFGS_COLORED and friction_mode != "current":
        # The thread-per-node body inlines the "current" isotropic Coulomb cone
        # clamp verbatim. Other friction modes are not ported to the colored
        # path; fail loudly rather than emit wrong arithmetic.
        raise NotImplementedError(
            "FEATHER_PGS_MFGS_COLORED (thread-per-node) supports friction_mode "
            f"'current' only; got '{friction_mode}'. Use the serial path "
            "(FEATHER_PGS_MFGS_COLORED=0) for other friction modes."
        )
...
"""
        ...
        # ── Colored CSR path, THREAD-PER-NODE (all-in, block_dim = 32, W=1) ──
        # Map ONE LANE (thread) per coloring NODE. A warp's 32 lanes process up
        # to 32 INDEPENDENT nodes of a color SIMULTANEOUSLY: lane L owns the
        # color-local node ordinals L, L+32, L+64, ... (stride 32 through the
        # color). The (validated) coloring guarantees no two nodes in a color
        # share a body, so the 32 lanes write DISJOINT s_v body slots -> no
        # atomics, no intra-color sync. The ONLY barrier is __syncwarp() BETWEEN
        # colors (color c+1 reads s_v written by color c).
        #
        # Each thread does its node's rows SERIALLY ON-THREAD:
        #   * J·v is a plain 6-iter (+6 for body_b) loop -- NO __shfl reduction.
        #   * the per-row projection (rt0 normal max(.,0) / rt4 bilateral /
        #     rt2 friction cone clamp vs sibling) is the verbatim serial
        #     arithmetic, run as ordinary scalar code on this one lane.
        #   * a node's normal + its rt2 friction rows are CONSECUTIVE in the CSR
        #     (normal first), so the cone-clamp's s_lam_mf[mf_par] read and the
        #     sibling read/write both stay within rows OWNED by this same lane
        #     -- NO cross-lane reach.
        #   * the v update writes this node's body DOFs directly from this lane.
        #
        # A node-start is any CSR row with mf_rt != 2 (rt0 normal / standalone);
        # rt2 friction rows ride with the preceding normal. Lane scans the
        # color's rows tracking node_ord; it processes a row iff its node is
        # owned (node_ord & 31 == lane). This is NOT bit-identical to serial
        # (serial-loop vs shuffle-tree differ in rounding) and NOT byte-clean for
        # n_colors=1 -- validate by FIDELITY + DETERMINISM, not bit-identity.
        #
        # NOTE: this thread-per-node body is single-thread scalar; it does NOT
        # use cur_Ja/cur_Jb/cur_MiJta/cur_MiJtb or a warp-reduced jv, so it
        # cannot reuse mf_row_body (which is warp-lane-striped). It re-expresses
        # the same per-row arithmetic directly. friction_mode is fixed to the
        # "current" isotropic Coulomb cone clamp here (guarded above).
        # Plain (non-f) string: the snippet has NO Python interpolations, so
        # single C braces are emitted verbatim (no {{ }} escaping needed).
        mf_main_loop = """
        ...
        for (int c = 0; c < mf_nc; c++) {
            ...
            // Walk this color's rows; lane owns node ordinals q with q%32==lane.
            ...
            for (int kk = cs; kk < ce; kk++) {
                ...
                int row_rt0 = meta.w & 0xFFFF;
                if (row_rt0 != 2) node_ord++;   // node-start (normal / standalone)
                // Only the lane that owns this node executes its rows.
                if ((node_ord & 31) != lane) continue;
                ...
                // ── J · v : plain on-thread loop (no __shfl) ──
                int row_mf6 = mf6_base + i * 6;
                float jv = 0.0f;
                if (dof_a >= 0) {
                    for (int k = 0; k < 6; k++) jv += mf_J_a.data[row_mf6 + k] * s_v[dof_a + k];
                }
                if (dof_b >= 0) {
                    for (int k = 0; k < 6; k++) jv += mf_J_b.data[row_mf6 + k] * s_v[dof_b + k];
                }
                ...
                if (mf_rt == 0) {
                    ...
                    if (radius <= 0.0f) {
                        ...
                        if (mag > radius) {
                            ...
                            // Sibling v update (this lane owns the sibling row).
                            int sib_packed_dofs = mf_meta.data[off_meta + sib * 4];
                            int sib_dof_a = sib_packed_dofs >> 16;
                            int sib_dof_b = (sib_packed_dofs << 16) >> 16;
                            int sib_mf6 = mf6_base + sib * 6;
                            if (sib_dof_a >= 0) {
                                for (int k = 0; k < 6; k++)
                                    s_v[sib_dof_a + k] += mf_MiJt_a.data[sib_mf6 + k] * sib_delta;
                            }
                            if (sib_dof_b >= 0) {
                                for (int k = 0; k < 6; k++)
                ...
                // ── V update : on-thread loop over this node's body DOFs ──
                if (delta_impulse != 0.0f) {
                    if (dof_a >= 0) {
                        for (int k = 0; k < 6; k++)
                            s_v[dof_a + k] += mf_MiJt_a.data[row_mf6 + k] * delta_impulse;
                    }
                    if (dof_b >= 0) {
                        for (int k = 0; k < 6; k++)
            ...
            __syncwarp();   // per-color barrier (thread-per-node, W=1: warp-local)
        }"""
...
"""
    ...
    if not MFGS_COLORED:
        ...
        name = f"pgs_solve_mf_gs_{max_constraints}_{mf_max_constraints}_{max_world_dofs}_{friction_mode}_colored{MAX_COLORS}_tpn"
