...
class SolverFeatherPGS(SolverBase):
    ...
    def _build_tpw_k1_inputs(
        ...
        *,
        ...
        # MF per-row scalar flats (W, M_MF) -> [i*W + world]. UNGATED: MF rows are
        # the live load on p10/p11 (up to mf_constraint_count); they are NOT dense
        # rows, so they must not be count-gated by the dense constraint_count.
        wp.launch(
            _tpw_k1_transpose_2d_to_inner_nogate, dim=(W, M_MF),
            inputs=[self.mf_impulses, W], outputs=[self._k1_mf_lam], device=device,
        )
        wp.launch(
            _tpw_k1_transpose_2d_to_inner_nogate, dim=(W, M_MF),
            inputs=[self.mf_row_mu, W], outputs=[self._k1_mf_row_mu], device=device,
        )
        # MF 6-component flats (W, M_MF, 6) -> [(i*6+k)*W + world].
        for src, dst in (
            ...
            wp.launch(
                ...
                inputs=[src, W], outputs=[dst], device=device,
            )
        # mf_meta (W, M_MF*4) -> [(i*4+f)*W + world].
        wp.launch(
            _tpw_k1_transpose_meta_to_inner, dim=(W, M_MF, 4),
            inputs=[mf_meta, W, M_MF], outputs=[self._k1_mf_meta], device=device,
        )
