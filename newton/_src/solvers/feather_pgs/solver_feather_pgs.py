...
class SolverFeatherPGS(SolverBase):
    ...
    def _build_tpw_k1_inputs(
        ...
        *,
        ...
        # Per-row dense scalar flats (W, M_D) -> [i*W + world].
        for src, dst, is_int in (
            ...
            wp.launch(kern, dim=(W, M_D), inputs=[src, W], outputs=[dst], device=device)

        # J/Y (W, M_D, D) -> (M_D*D, W).
        wp.launch(
            _tpw_k1_transpose_jy_to_inner, dim=(W, M_D, D),
            inputs=[self.J_world, W, D], outputs=[self._k1_J], device=device,
        )
        wp.launch(
            _tpw_k1_transpose_jy_to_inner, dim=(W, M_D, D),
            inputs=[self.Y_world, W, D], outputs=[self._k1_Y], device=device,
        )

        # MF per-row scalar flats (W, M_MF) -> [i*W + world].
        wp.launch(
