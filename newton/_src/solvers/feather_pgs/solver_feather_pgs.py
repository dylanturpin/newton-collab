...
class SolverFeatherPGS(SolverBase):
    ...
    def _launch_matrix_free_gs_solve(
        ...
        *,
        ...
        # ----------------------------------------------------------------------
        # Solve-only micro-bench (FEATHER_PGS_MFGS_SOLVE_BENCH=1). After the pile
        # settles, replay the EXACT solve-kernel schedule N times with a device
        # sync per replay and print the MEDIAN per-replay ms, clearly tagged so it
        # parses from the probe log. Runs ONCE per process, only when NOT inside a
        # CUDA-graph capture (a real-launch + sync bench is illegal under capture).
        # Variant (serial vs colored W) is whatever this solver was built with.
        # ----------------------------------------------------------------------
        if (
            os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH", "0").lower() in {"1", "true", "yes", "on"}
            and not getattr(self, "_mfgs_solve_bench_done", False)
            and self._step >= int(os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH_STEP", "150"))
        ):
            dev = self.model.device
            capturing = False
            try:
                capturing = bool(getattr(wp.get_device(str(dev)), "is_capturing", False))
            except Exception:
                capturing = False
            if not capturing:
                ...
                self._mfgs_solve_bench_done = True
                n_launches = max(int(os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH_N", "200")), 1)
                n_warmup = max(int(os.environ.get("FEATHER_PGS_MFGS_SOLVE_BENCH_WARMUP", "30")), 0)
                variant = (
                    f"colored_W{self._mfgs_warps}" if self._mfgs_colored else "serial"
                )
                try:
                    ...
                    for _ in range(n_launches):
                        ...
                        _run_schedule()
                        wp.synchronize_device(dev)
                        _samples.append((_time.perf_counter() - _t0) * 1e3)
                    _med = _stats.median(_samples)
                    _p10 = _stats.quantiles(_samples, n=10)[0] if len(_samples) >= 10 else min(_samples)
                    _mn = min(_samples)
                    print(
                        f"[SOLVE-BENCH] variant={variant} colored={int(self._mfgs_colored)} "
                        f"W={self._mfgs_warps} block_dim={mf_block_dim} worlds={self.world_count} "
                        f"iters={iterations} schedule={self.pgs_schedule} "
                        f"step={self._step} launches={n_launches} warmup={n_warmup} "
                        f"median_ms={_med:.5f} min_ms={_mn:.5f} p10_ms={_p10:.5f}",
                        flush=True,
                    )
                except Exception as _e:  # never let the bench break the run
