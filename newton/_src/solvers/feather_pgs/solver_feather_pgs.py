...
class SolverFeatherPGS(SolverBase):
    ...
    def reset(
        ...
        """Invalidate cached dynamics and clear persistent solver state for reset worlds.

        The authored simulation state is preserved. Solver-owned state derived
        from it, dense, matrix-free, and propagation impulse history, and carried
        friction anchors are cleared.
...
"""
    ...
    if not shared_metadata:
        name += "_gmeta"
    if skip_local_internal_worlds:
        name += f"_local_fallback{local_internal_max_constraints}"
    if contact_torsion:
        name += "_torsion"
