...
class SolverFeatherPGS(SolverBase):
    ...
    def _record_stage3_dependency(self) -> None:
        if not self.model.device.is_cuda or self.v_out_snap is None:
            return
        # The former always-on stage-3 debug copies materialized v_hat before
        # later contact/solve stages consumed it. Keep that CUDA dependency
        # explicit without retaining diagnostic buffers; v_out_snap is
        # overwritten again before every semantic use.
    ...
    def step(
        ...
        with wp.ScopedTimer("S3_Trisolve_Vhat", print=False, use_nvtx=self._nvtx, synchronize=self._nvtx):
            ...
            self._record_stage3_dependency()
