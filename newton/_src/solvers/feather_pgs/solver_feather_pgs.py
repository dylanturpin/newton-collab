...
class SolverFeatherPGS(SolverBase):
    ...
    def _record_stage3_dependency(self) -> None:
        if not self.model.device.is_cuda:
            return
        # The former always-on stage-3 debug copies also inserted a concrete
        # CUDA stream operation after producing v_hat. Keep that dependency
        # explicit without materializing diagnostic arrays during normal runs.
