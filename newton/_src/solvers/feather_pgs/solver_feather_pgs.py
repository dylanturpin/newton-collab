...
class SolverFeatherPGS(SolverBase):
    ...
    def _classify_active_velocity_limits(self, model):
        def _has_finite_positive(values) -> bool:
            if values is None:
                return False
            values_np = values.numpy()
            return bool(np.any((values_np > 0.0) & np.isfinite(values_np)))

        self._has_joint_velocity_limits = (
            self.enable_joint_velocity_limits
            and model.joint_dof_count > 0
            and _has_finite_positive(getattr(model, "joint_velocity_limit", None))
        )
