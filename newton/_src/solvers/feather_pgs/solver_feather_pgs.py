...
"""
    ...
    # perf (opt-in): software-pipeline the mf_rt==4 velocity-limit loop, mirroring the
    # main mf loop's prefetch. The vel-limit loop is the only mf loop NOT pipelined: it
    # loads mf_meta then (for rt==4 rows) mf_J_a/b + mf_MiJt_a/b inline. Hoist the next
    # row's loads into registers while the current row computes. CRITICAL for bit-identity:
    # advance the pipeline (prefetch row i+1) BEFORE the `mf_rt != 4 continue`, so skipped
    # rows still issue their loads — same flat indices, same arithmetic, only load *timing*
    # moves earlier. Opt-in via FEATHER_PGS_MFGS_VLIMIT_PIPELINE=1 (default off = original).
    if os.environ.get("FEATHER_PGS_MFGS_VLIMIT_PIPELINE", "").strip().lower() in ("1", "true", "yes", "on"):
        _vlim_orig = """        if (row_phase == 0 || row_phase == 2 || row_phase == 5) {
            for (int i = 0; i < m_mf; i++) {
                int4 meta = *reinterpret_cast<const int4*>(&mf_meta.data[off_meta + i * 4]);
                int packed_tp = meta.w;
                int mf_rt = packed_tp & 0xFFFF;
        ...
        _vlim_new = """        if (row_phase == 0 || row_phase == 2 || row_phase == 5) {
        ...
        if _vlim_orig not in snippet:
            raise RuntimeError("FEATHER_PGS_MFGS_VLIMIT_PIPELINE: vel-limit loop head not found")
        snippet = snippet.replace(_vlim_orig, _vlim_new)
        # Consume prefetched MiJt in the V-update (drop the inline global loads + row_mf6).
        _vlim_vupd_orig = """                if (delta_impulse_mf_vlim != 0.0f) {
                    if (lane < 6 && dof_a >= 0) {
                        s_v[dof_a + lane] += mf_MiJt_a.data[row_mf6 + lane] * delta_impulse_mf_vlim;
                    }
                    if (lane >= 6 && lane < 12 && dof_b >= 0) {
        ...
        _vlim_vupd_new = """                if (delta_impulse_mf_vlim != 0.0f) {
                    if (lane < 6 && dof_a >= 0) {
        ...
        if _vlim_vupd_orig not in snippet:
            raise RuntimeError("FEATHER_PGS_MFGS_VLIMIT_PIPELINE: vel-limit V-update not found")
        snippet = snippet.replace(_vlim_vupd_orig, _vlim_vupd_new)
