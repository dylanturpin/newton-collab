...
class TestViewerRerunHidden(unittest.TestCase):
    ...
    def test_set_rr_time_uses_legacy_rerun_timeline_api(self):
        """Older Rerun SDKs use explicit sequence/seconds timeline setters."""
        from newton._src.viewer.viewer_rerun import ViewerRerun

        legacy_rr = types.SimpleNamespace(
            set_time_sequence=Mock(),
            set_time_seconds=Mock(),
        )

        with patch("newton._src.viewer.viewer_rerun.rr", legacy_rr):
            ViewerRerun._set_rr_time("sim_step", sequence=7)
            ViewerRerun._set_rr_time("time", timestamp=0.14)

        legacy_rr.set_time_sequence.assert_called_once_with("sim_step", 7)
        legacy_rr.set_time_seconds.assert_called_once_with("time", 0.14)

    def test_record_to_rrd_can_skip_live_server_on_legacy_rerun_sdk(self):
        """Offline RRD recording should not require newer live web-server APIs."""
        from newton._src.viewer.viewer_rerun import ViewerRerun
        ...
        legacy_rr = types.SimpleNamespace(
            ...
            set_time_sequence=Mock(),
            set_time_seconds=Mock(),
            Mesh3D=Mock(return_value=Mock()),
        )
        mock_rrb = Mock()
        mock_rrb.Blueprint = Mock(return_value=Mock())
        mock_rrb.Horizontal = Mock(return_value=Mock())
        mock_rrb.Spatial3DView = Mock(return_value=Mock())
        mock_rrb.TimePanel = Mock(return_value=Mock())
        mock_rrb.TimeSeriesView = Mock(return_value=Mock())

        with patch("newton._src.viewer.viewer_rerun.rr", legacy_rr):
            with patch("newton._src.viewer.viewer_rerun.rrb", mock_rrb):
                with patch("newton._src.viewer.viewer_rerun.is_jupyter_notebook", return_value=False):
                    viewer = ViewerRerun(record_to_rrd="/tmp/p9.rrd", serve_web_viewer=True)

        self.assertIsNotNone(viewer)
        legacy_rr.save.assert_called_once()
        legacy_rr.spawn.assert_not_called()
        legacy_rr.set_time_seconds.assert_called_once_with("time", 0.0)
